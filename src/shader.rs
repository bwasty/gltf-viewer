...
use crate::platform::{compile_shader_and_get_id,read_vertex_code,read_fragment_code,UniformHelpers};
...
pub struct Shader {
    pub id: u32,
    pub uniform_location_cache: HashMap<&'static str, i32>
}
...
impl Shader {
    ...
    pub fn from_source(vertex_code: &str, fragment_code: &str, defines: &[String]) -> Shader {
        ...
        unsafe {
            shader.id = compile_shader_and_get_id(&vertex_code.to_string(), &fragment_code.to_string()).unwrap();
        }
